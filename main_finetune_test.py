...
from ADNIDataset_lin import make_dataset
...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_path_train', default='/datasets01/imagenet_full_size/061417/', type=str,
                        help='dataset path')
...
def main(args):
    ...
    transform_train = transforms.Compose([
            #transforms.RandomResizedCrop(args.input_size, scale=(0.2, 1.0), interpolation=3),  # 3 is bicubic mask075
            transforms.RandomResizedCrop(args.input_size),
            transforms.Grayscale(num_output_channels = 1),
            transforms.ToTensor()])
    dataset_train = make_dataset(args.data_path_train,args.label_csv,transform_train,args.sample_hard,args.sample_hard_level)
    transform_val = transforms.Compose([
            #transforms.RandomResizedCrop(args.input_size, scale=(0.2, 1.0), interpolation=3),  # 3 is bicubic mask075
            transforms.RandomResizedCrop(args.input_size),
            transforms.Grayscale(num_output_channels = 1),
            transforms.ToTensor()])
    dataset_val = make_dataset(args.data_path_train,args.label_csv,transform_val,args.sample_hard,args.sample_hard_level)
    ...
    model.second_fc = nn.Linear(864,256)
    model.last_fc = nn.Linear(256,3)
    model.to(device)
